...
def train_model(model, adam, train_data, dev_data, tokenizer, device, args):
    ...
    for epoch in range(args.num_epoch):
        ...
        # cur 是一个字典，表示当前训练批次的数据，包含json数据的键
        for i, cur in enumerate(tqdm(train_data, desc='Epoch {}:'.format(epoch))):
            ...
            # 计算四个参考摘要的损失
            reference_losses = []
            for j, reference_summary in enumerate(cur['summaries']):  # 迭代所有参考摘要
                reference_labels = reference_summary[:, 1:].reshape(-1)[mask]

                # 计算参考摘要的损失
                ref_loss = loss_fct(prob, reference_labels)
                reference_losses.append(ref_loss)
                print(f"Reference {j + 1} Loss: {ref_loss.item()}")
                # writer.add_scalar(f'Train/Reference_{j + 1}_Loss', ref_loss.item(), epoch * len(train_data) + i)

            loss = loss + 0.5 * sum(reference_losses)

            epoch_train_loss += loss.item()
            if i % 100 == 0:
                print("Iter {}:  Training Loss: {}".format(i, loss.item()))
                # 记录训练损失到 TensorBoard
                print("Logging training loss to TensorBoard")
                writer.add_scalar('Train/Loss', loss.item(), epoch * len(train_data) + i)
        ...
        writer.add_scalar('Validation/ROUGE-1', scores['rouge-1'], epoch)
        writer.add_scalar('Validation/ROUGE-2', scores['rouge-2'], epoch)
        writer.add_scalar('Validation/ROUGE-L', scores['rouge-l'], epoch)
...
def init_argument():
    ...
    parser.add_argument('--train_data', default='C:/Users\Administrator\Desktop/train3.tsv')  # 训练集
    parser.add_argument('--dev_data', default='C:/Users\Administrator\Desktop/dev.tsv')  # 验证集
    parser.add_argument('--pretrain_model', default='F:/chinese_t5_pegasus_base')  # 预训练模型位置
    parser.add_argument('--model_dir', default='F:/output')  # 模型权重文件位置
    ...
    parser.add_argument('--log_dir', default='F:/logs', help='log directory for TensorBoard')  # 日志目录
