...
def train_model(model, adam, train_data, dev_data, tokenizer, device, args):
    ...
    # 定义类别权重，这里假设您有4个类别，您需要根据实际情况进行调整
    # class_weights = torch.tensor([1.0, 2.0, 2.0, 2.0]).to(device)  # 示例权重，您可以根据需要进行调整
...
def init_argument():
    ...
    parser.add_argument('--model_dir', default='F:/output')# 训练后的模型权重文件位置
